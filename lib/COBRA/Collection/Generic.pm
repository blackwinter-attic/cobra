package COBRA::Collection::Generic;

=head1 NAME

COBRA::Collection::Generic - Generic collection class

=head1 VERSION

This documentation refers to COBRA::Collection::Generic version 0.0.1
...
sub parse_records {
  ...
  if ($field_re =~ s{\A([^|]+)\|([^|]+)\|?}{}) {
    ...
    croak("Couldn't extract id and class field from field list '$field_re'");
  }

  # any fields left?
  croak("No fields left in field list after extracting id and class field")
    unless length $field_re;

  # build regexp to extract the record identifier, captured by parentheses
  $id_re    = qr{\A$id_re$delimiter_re(.*)}o;

  # build regexp to extract the assigned classes
  $class_re = qr{\A$class_re$delimiter_re}o;
...
# get_* dummies to satisfy COBRA::Collection's sanity checks:
sub get_id_re {
  # dummy
}
