package COBRA::Collection::STN;

=head1 NAME

COBRA::Collection::STN - Abstract base class for collections from STN records

=head1 VERSION

This documentation refers to COBRA::Collection::STN version 0.0.1
...
my %start_record_re : ATTR(:get<start_record_re>);
my %end_record_re   : ATTR(:get<end_record_re>  );
my %delimiter_re    : ATTR(:get<delimiter_re>   );
my %continuation_re : ATTR(:get<continuation_re>);
...
sub START {
  ...
  # regular expression that marks the start of a record (excluded)
  $start_record_re{$ident} = qr{\AL\d+\s+ANSWER\s+\d+\s+OF\s+\d+\s+.*\s+STN}o;
  # regular expression that marks the end of a record (excluded)
  $end_record_re{$ident}   = qr{\A\s*\z}o;
  # regular expression that separates a field label from its content
  $delimiter_re{$ident}    = qr{\s+}o;
  # regular expression that marks continued field contents
  $continuation_re{$ident} = qr{\A\s+}o;
}
...
sub parse_records {
  ...
  my $start_record_re = $self->get_start_record_re();
  my $end_record_re   = $self->get_end_record_re();
  my $delimiter_re    = $self->get_delimiter_re();
  my $continuation_re = $self->get_continuation_re();
  my $id_re           = $self->get_id_re();
  my $class_re        = $self->get_class_re();
  my $class_split_re  = $self->get_class_split_re();

  # build regexp to match desired fields (and capture that field)
  ...
  foreach my $infile (@{$data_files_ref}) {
    ...
    foreach my $line (@lines) {
      ...
      # records are between $START_RECORD_RE and $END_RECORD_RE
      if    ($line =~ $start_record_re) {
        # entering record
        $in_record = 1;

        next LINE;  # exclude this line
      }
      elsif ($line =~ $end_record_re) {
        # leaving record
        $in_record = 0;
      ...
      elsif ($line =~ $continuation_re && length $curfield) {
        # continued content from previous field
        (my $content = $line) =~ s{$continuation_re}{};
        $buffer[-1] .= q{ } . $content;
      }
      else {
...
=head1 SYNOPSIS
    ...
    Don't use COBRA::Collection::STN directly, use an appropriate subclass instead.
